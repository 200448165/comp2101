...
hstnm=$(hostname)
LnAdrs=$(ip a s $(ip a |awk '/: e/{gsub(/:/,"");print $2}')|awk '/inet /{gsub(/\/.*/,"");print $2}')
Lnhst=$(getent hosts $LnAdrs | awk '{print $2}')
...
RTAD=$(ip r | awk '/via /{gsub(/\/.*/,"");print $3}')
RTHN=$(getent hosts $RTAD | awk '{print $2}')
